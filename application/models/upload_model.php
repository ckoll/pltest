...
//        imagedestroy($from);
    ...
    public function get_photo_comments($for_page, $page, $id) {
        $begin = $for_page * $page;
        return $this->db->query('SELECT SQL_CALC_FOUND_ROWS *, users.username, photo_comments.id comment_id FROM photo_comments LEFT JOIN users ON photo_comments.uid = users.id WHERE photo_id=' . $id . ' ORDER BY photo_comments.id DESC LIMIT ' . $begin . ',' . $for_page)->result_array();
    }
